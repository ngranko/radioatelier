...
describe('patchObjectRecords', () => {
    ...
    it('indexes the post-write state across object and map point changes', async () => {
        const {ctx, db, scheduler} = createMockCtx();
        ...
        expect(scheduler.runAfter).toHaveBeenCalledWith(0, internal.typesense.updateInTypesense, {
            object: {
                id: 'object-1',
                name: 'Plaque',
                address: 'Tverskaya 1',
