...
type MapPointFields = MapPointCoreData;
...
export function filterChangedPatch<T extends object>(record: T, patch: T): Partial<T> {
    const result: Partial<T> = {};
    for (const key of Object.keys(patch) as (keyof T)[]) {
        if (!arePatchValuesEqual(record[key], patch[key])) {
