...
export type ObjectTarget = ObjectAggregate & {
    marker: Doc<'markers'>;
};
...
async function scheduleSearchUpdate(
    ...
    const nextObject = {...target.object, ...objectPatch};
    const nextMapPoint = {...target.mapPoint, ...mapPointPatch};
    const category =
        nextObject.categoryId !== target.category._id
            ? await requireCategory(ctx, nextObject.categoryId)
            : target.category;
