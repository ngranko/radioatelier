<script lang="ts">
    ...
    async function syncPanoramaToMiniMapCenter() {
        if (!miniMap || !panorama || !shouldSyncPanoramaFromMiniMap) {
            return;
        }
        if (isMovingPanorama) {
            // if we got here it means that another sync is already in-flight and we allow it to finish before starting the next one (in finally in this very function)
            queuedSyncAfterMove = true;
            return;
        }
        ...
        try {
            ...
            if (generation !== syncGeneration) {
                // if generations are not equal it means that another sync is inevitable (a user started dragging a minimap again), so there's no reason to apply this one
                return;
            }
            ...
            isMovingPanorama = false;
            if (queuedSyncAfterMove || shouldSyncPanoramaFromMiniMap) {
                queuedSyncAfterMove = false;
                shouldSyncPanoramaFromMiniMap = true;
                schedulePanoramaSync();
            } else {
