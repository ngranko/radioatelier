...
func (p PageProperties) FillFromObject(ctx context.Context, obj *ent.Object) PageProperties {
    ...
    objUsr, err := obj.QueryObjectUser().
        Where(objectuser.UserIDEQ(puuid.ID(ctx.Value(c.SubjectUser).(*ent.User).ID))).
        First(ctx)
    if err == nil {
        p.SetIsVisited(objUsr.IsVisited)
        p.SetLastVisited(objUsr.LastVisit)
    }
...
func (p PageProperties) GetIsVisited() bool {
    return p[IsVisited].(*property.CheckboxProperty).GetValue()
}

func (p PageProperties) SetIsVisited(value bool) {
    p[IsVisited].(*property.CheckboxProperty).SetValue(value)
}

func (p PageProperties) GetLastVisited() *time.Time {
    return p[LastVisited].(*property.DateProperty).GetValue()
}

func (p PageProperties) SetLastVisited(value *time.Time) {
    p[LastVisited].(*property.DateProperty).SetValue(value)
}
