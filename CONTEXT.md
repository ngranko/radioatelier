...
- **Taxonomy** — the umbrella term for the three vocabularies an Object is classified by: category, tag, private tag. Used where a rule holds for all three (the admin `/taxonomies` page, `helpers/taxonomyRef.ts`, `helpers/taxonomyEditor.ts`).
- **Taxonomy editor** — `src/convex/helpers/taxonomyEditor.ts`. The admin-side counterpart of the Object writer: renames a taxonomy or deletes it, optionally moving every Object that uses it to another taxonomy of the same kind. Bulk rewrites of `objects` / `markers` / `objectPrivateTags` live in `helpers/objectTaxonomy.ts` (which also owns archive-wide usage counting), and the resulting search and Notion work is batched by `helpers/taxonomyFanout.ts` instead of being scheduled per Object. See [docs/taxonomy-management.md](docs/taxonomy-management.md).
...
- **Inbound apply patch** (`AppSyncApplyPatch` in code) — the writable projection of a Notion→app diff that inbound sync actually applies. Raw diffs retain `null` for emptied Notion fields (visible to the Sync audit); the inbound decision is the single owner of keep-vs-clear and turns those empties into writes for nullable Object fields. Empty `name` / `categoryName` are refused instead (sync error recorded when a linked Object exists, then the inbound action throws) because those fields are required in the app.
