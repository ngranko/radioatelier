...
- **End** — teardown hangs off the animation's own `animationend` / `animationcancel`. A timer strips the class before the animation has played whenever the draw is late, which is what happens when a pan brings hundreds of markers in at once.
