...
`createObjectRecords(ctx, ownerId, data)` inserts `mapPoints`, `objects`, and `markers`, then schedules `internal.typesense.createInTypesense`. CSV import and inbound Notion create both use this path — import indexing (#93) depends on create going through the writer, not ad-hoc inserts.
